...
class OptimizationModel:
    ...
    def generate_workspace(self):
        ...
        q_in_x = linspace(self.pos_bounds[0,0]+1.85,self.pos_bounds[0,1],50)
        q_in_y = linspace(self.pos_bounds[1,0],self.pos_bounds[1,1],50)
        ...
        for i in range(len(q_in_x)):
            ...
            for j in range(len(q_in_y)):
                ...
                if (Gamma_min <= 0):
                    ...
                    print('NOT FEASIBLEEEEEEEEEEEEEEEEEEEEEEEEEE')
                    scaling_factor =  0.005
                    cartesian_desired_vertices_plt = (scaling_factor*self.cartesian_desired_vertices) + array([[x_in,y_in]])

                    plt.plot(cartesian_desired_vertices_plt[:,0], cartesian_desired_vertices_plt[:,1],color = 'red')
                    ...
                    plt.plot(polytope_vertices_plt[:,0], polytope_vertices_plt[:,1],color = 'r')
                ...
                if (Gamma_min_softmax > 0):
                    ...
                    #input('stop here')
                    q_estimated = vstack((q_estimated,q))
                plt.pause(0.01)
                plt.gcf().clear()
        
        q_actual = q_actual[1:,:]
        q_estimated = q_estimated[1:,:]

        return q_actual, q_estimated
