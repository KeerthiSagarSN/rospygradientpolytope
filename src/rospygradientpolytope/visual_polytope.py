...
import pycapacity
from numpy import hstack,vstack,array,dot,transpose,append
...
def pycapacity_polytope(JE,qdot_max,qdot_min):
    ...
    polytope_verts,polytope_faces = velocity_polytope_withfaces(JE[0:3,:],qdot_max,qdot_min)
...
def plot_polytope_actual(ax,JE,active_joints,cartesian_dof_input,qdot_min,qdot_max,cartesian_desired_vertices,sigmoid_slope):
    ...
    ax.cla()
    h_plus,h_plus_hat,h_minus,h_minus_hat,p_plus,p_minus,n_k, Nmatrix, Nnot = get_polytope_hyperplane(JE,active_joints,cartesian_dof_input,qdot_min,qdot_max,cartesian_desired_vertices,sigmoid_slope)
    A = vstack((n_k,-n_k))
    ...
    for s in hull2.simplices:
        ...
        ax.plot(polytope_vertices[s, 0], polytope_vertices[s, 1], polytope_vertices[s, 2], "k-")
