...
def hyperplane_gradient(JE,H,n_k,Nmatrix, Nnot,h_plus_hat,h_minus_hat,p_plus_hat,\
    ...
    for normal_index in range(len(Nmatrix)):
        ...
        dn_dq[normal_index,:] = gradient_functions.normal_gradient(twist_index_1, twist_index_2,JE,H, test_joint)
        ...
        for index_projected_screw in range(len(Nnot[normal_index])):
            ...
            d_nT_vk_dq = gradient_functions.normal_twist_projected_gradient(twist_index_1,twist_index_2,twist_index_projected,JE,H,test_joint)
            ...
            d_sig_nT_vk_dq = gradient_functions.sigmoid_gradient(twist_index_1, twist_index_2, twist_index_projected,JE,H,test_joint,sigmoid_slope)
            d_negative_sig_nT_vk_dq = gradient_functions.sigmoid_gradient(twist_index_1, twist_index_2, twist_index_projected,JE,H,test_joint, -sigmoid_slope)
...
def Gamma_hat_gradient_joint(JE,H,n_k,Nmatrix, Nnot,h_plus_hat,h_minus_hat,p_plus_hat,\
                        p_minus_hat,qdot_min,qdot_max,\
                            cartesian_desired_vertices,test_joint,sigmoid_slope):
    
    from numpy import empty,shape,cross,zeros,dot,transpose,matmul,array,hstack
    from numpy.linalg import norm
    from linearalgebra import V_unit, check_ndarray
    from robot_functions import exp_normalize
...
def Gamma_hat_gradient(JE,H,n_k,Nmatrix, Nnot,h_plus_hat,h_minus_hat,p_plus_hat,\
                        ...
                        qdot_min,qdot_max,sigmoid_slope):
